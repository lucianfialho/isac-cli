 ...
 *
 * Runs in the page context via chrome-devtools `evaluate_script`.
 * Detects CSS animations, transitions, scroll-triggered animations,
 * and JS animation libraries on the page.
 *
 * Returns a structured catalog of all detected animations.
 */
...
(function detectAnimations() {
  ...
  // --- 1. Extract @keyframes from all stylesheets ---
  ...
  // --- 2. Detect CSS animations on elements ---
  ...
  // --- 3. Detect CSS transitions on elements ---
  ...
  // --- 4. Detect Web Animations API usage ---
  ...
  // --- 5. Detect scroll-triggered animation patterns ---
  function detectScrollAnimations() {
    const results = [];

    // Check for common scroll-reveal CSS classes
    const scrollClasses = [
      'aos-animate', 'aos-init',
      'wow', 'animated',
      'scroll-reveal', 'reveal',
      'fade-in', 'fade-up', 'fade-down', 'fade-left', 'fade-right',
      'slide-in', 'slide-up', 'slide-down',
      'zoom-in', 'zoom-out',
      'is-visible', 'in-view',
    ...
    // Check for elements with data-aos attributes (AOS library)
    document.querySelectorAll('[data-aos]').forEach(el => {
      results.push({
        ...
        offset: el.dataset.aosOffset
      });
    });

    // Check for elements with data-scroll attributes (Locomotive Scroll)
    ...
    // Check for GSAP ScrollTrigger markers
  ...
  // --- 6. Detect animation libraries loaded on the page ---
  function detectLibraries() {
    ...
    if (window.anime) {
      libraries.push({ name: 'anime.js', version: 'detected' });
    }
    if (window.__FRAMER_MOTION__) {
      libraries.push({ name: 'framer-motion', version: 'detected' });
    }
    if (window.Motion || document.querySelector('[data-motion]')) {
      libraries.push({ name: 'motion', version: 'detected' });
    }
    if (window.ScrollReveal) {
      libraries.push({ name: 'scrollreveal', version: 'detected' });
    }
    if (window.AOS) {
      libraries.push({ name: 'aos', version: 'detected' });
    }
    if (window.Lenis || window.LocomotiveScroll) {
      libraries.push({ name: 'smooth-scroll', version: 'detected' });
    }
    if (window.lottie || window.bodymovin) {
      libraries.push({ name: 'lottie', version: 'detected' });
    }

    // Check script tags for library URLs
  ...
  // --- 7. Generate the complete catalog ---
